...
/**
 * @package  rehomes-core
 * @category Plugins
 * @author   WpOpal
 * Plugin Name: Rehomes Core
 ...
 * used, this is required. Version: 2.3.2 Author: WpOpal Author URI:  http://www.wpopal.com/ License: GNU/GPLv3
 * http://www.gnu.org/licenses/gpl-3.0.html
 */

final class RehomesCore {
	/**
	 * @var RehomesCore
	 */
	...
	public function setup_constants() {
		if ( ! defined( 'REHOME_CORE_VERSION' ) ) {
			define( 'REHOME_CORE_VERSION', '2.3.2' );
		}

		if ( ! defined( 'REHOME_CORE_PLUGIN_DIR' ) ) {
			define( 'REHOME_CORE_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
		}

		if ( ! defined( 'REHOME_CORE_PLUGIN_URL' ) ) {
			define( 'REHOME_CORE_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
		}

		if ( ! defined( 'REHOME_CORE_PLUGIN_FILE' ) ) {
			define( 'REHOME_CORE_PLUGIN_FILE', __FILE__ );
		}
	}
	...
	private function load_textdomain() {
		$lang_dir      = dirname( plugin_basename( REHOME_CORE_PLUGIN_FILE ) ) . '/languages/';
		$lang_dir      = apply_filters( 'osf_languages_directory', $lang_dir );
		$locale        = apply_filters( 'plugin_locale', get_locale(), 'rehomes-core' );
		$mofile        = sprintf( '%1$s-%2$s.mo', 'rehomes-core', $locale );
		$mofile_local  = $lang_dir . $mofile;
		$mofile_global = WP_LANG_DIR . '/rehomes-core/' . $mofile;

		if ( file_exists( $mofile_global ) ) {
			load_textdomain( 'rehomes-core', $mofile_global );
		} else {
			if ( file_exists( $mofile_local ) ) {
				load_textdomain( 'rehomes-core', $mofile_local );
			} else {
				load_plugin_textdomain( 'rehomes-core', false, $lang_dir );
			}
		}
	}
	...
	public function plugin_update() {
		...
		Puc_v4_Factory::buildUpdateChecker(
			'http://source.wpopal.com/rehomes/dummy_data/update-plugin.json',
			__FILE__,
			'rehomes-core'
		);
	}
...
//		require_once 'inc/vendors/cmb2/custom-fields/slider/slider.php';
	...
	public static function getInstance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof RehomesCore ) ) {
			self::$instance = new RehomesCore();
		}
	...
	public function init_theme_support() {
        ...
        require_once 'inc/vendors/vertical-line/vertical-line.php';

		new OSF_Metabox();
	}
}

return RehomesCore::getInstance();
