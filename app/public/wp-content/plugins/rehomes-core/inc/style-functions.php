...
function osf_theme_custom_css() {
    $a1root      = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/a1root.php';
    $grid        = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/grid.php';
    $button      = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/button.php';
    $error_404   = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/error-404.php';
    $heading     = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/heading.php';
    $main_layout = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/main-layout.php';
    $page_bg     = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/page-bg.php';
    $page_title  = include trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'inc/customize/page-title.php';
    $css         = <<<CSS
{$a1root}
{$grid}
{$error_404}
...
//        $css .= '.site-footer {padding-top:' . $footer_padding_top . 'px!important;}';
    ...
    if (is_singular('osf_portfolio')) {
        $page_title_bar = '';
        if (get_post_meta(get_the_ID(), 'osf_portfolio_banner', 1)) {
            $page_title_bar .= "background:linear-gradient(0deg,rgba(23, 62, 98, 0.4),rgba(23, 62, 98, 0.4)), url(" . get_post_meta(get_the_ID(), 'osf_portfolio_banner', 1) . ");";
            $css            .= '.page-title-bar {' . $page_title_bar . '}';
        }
    }
