...
class OSF_Elementor_Image_Layers_Widget extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section('img_layers_content',
            [
                'label' => esc_html__('Images', 'rehomes-core'),
            ]
        );
        ...
        $repeater->add_control('layers_select',
            [
                'label'   => esc_html__('Select Content', 'rehomes-core'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'image' => esc_html__('Image Content', 'rehomes-core'),
                    'text'  => esc_html__('Text Content', 'rehomes-core'),
                ],
                'default' => 'image'
            ]
        );
        ...
        $repeater->add_control('img_layers_image',
            [
                'label'     => esc_html__('Upload Image', 'rehomes-core'),
        ...
        $repeater->add_control('img_layers_alt',
            [
                'label'       => esc_html__('Alt', 'rehomes-core'),
        ...
        $repeater->add_control('text_layers',
            [
                'label'     => esc_html__('Content', 'rehomes-core'),
                'type'      => Controls_Manager::TEXTAREA,
                'default'   => __('Lorem ipsum dolor sit amet', 'rehomes-core'),
                'condition' => [
                    'layers_select' => 'text'
                ]
            ]
        ...
        $repeater->add_control('text_color',
            [
                'label'     => esc_html__('Color', 'rehomes-core'),
        ...
        $repeater->add_control('img_layers_position',
            [
                'label'   => esc_html__('Position', 'rehomes-core'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'relative' => esc_html__('Relative', 'rehomes-core'),
                    'absolute' => esc_html__('Absolute', 'rehomes-core'),
                ],
                'default' => 'relative'
            ]
        );
        ...
        $repeater->add_responsive_control('img_layers_hor_position',
            [
                'label'       => esc_html__('Horizontal Offset', 'rehomes-core'),
                'type'        => Controls_Manager::SLIDER,
                'description' => esc_html__('Mousemove Interactivity works only with pixels', 'rehomes-core'),
                'size_units'  => ['px', "em", '%'],
                'range'       => [
                    'px' => [
                        'min' => -200,
        ...
        $repeater->add_responsive_control('img_layers_ver_position',
            [
                'label'       => esc_html__('Vertical Offset', 'rehomes-core'),
                'description' => esc_html__('Mousemove Interactivity works only with pixels', 'rehomes-core'),
                'type'        => Controls_Manager::SLIDER,
                'size_units'  => ['px', "em", '%'],
                'range'       => [
        ...
        $repeater->add_control('img_layers_link_selection',
            [
                'label'       => esc_html__('Link Type', 'rehomes-core'),
                'type'        => Controls_Manager::SELECT,
                'options'     => [
        ...
        $repeater->add_control('img_layers_rotate',
            [
                'label' => esc_html__('Rotate', 'rehomes-core'),
                'type'  => Controls_Manager::SWITCHER,
            ]
        );
        ...
        $repeater->add_control('img_layers_rotatex',
            [
                'label'       => esc_html__('Degrees', 'rehomes-core'),
                'type'        => Controls_Manager::NUMBER,
                'description' => esc_html__('Set rotation value in degress', 'rehomes-core'),
                'min'         => -180,
                'max'         => 180,
                'condition'   => [
        ...
        $repeater->add_control('img_layers_animation',
            [
                'label'              => esc_html__('Entrance Animation', 'rehomes-core'),
        ...
        $repeater->add_control('img_layers_animation_duration',
            [
                ...
                'default'   => '',
                'options'   => [
                    'slow' => __('Slow', 'rehomes-core'),
                    ''     => __('Normal', 'rehomes-core'),
                    'fast' => __('Fast', 'rehomes-core'),
                ],
                'condition' => [
                    'img_layers_animation_switcher' => 'yes',
        ...
        $repeater->add_control('img_layers_animation_delay',
            [
                'label'              => esc_html__('Animation Delay', 'rehomes-core') . ' (s)',
                'type'               => Controls_Manager::NUMBER,
                'default'            => '',
                'min'                => 0,
                'step'               => 0.1,
        ...
        $repeater->add_control('img_layers_mouse',
            [
                'label'       => esc_html__('Mousemove Interactivity', 'rehomes-core'),
                'type'        => Controls_Manager::SWITCHER,
                'description' => esc_html__('Enable or disable mousemove interaction', 'rehomes-core'),
            ]
        );

        $repeater->add_control('img_layers_mouse_type',
            [
                'label'       => esc_html__('Interactivity Style', 'rehomes-core'),
                'type'        => Controls_Manager::SELECT,
                'options'     => [
                    'parallax' => esc_html__('Mouse Parallax', 'rehomes-core'),
                    'tilt'     => esc_html__('Tilt', 'rehomes-core'),
                ],
                'default'     => 'parallax',
                'label_block' => true,
                'condition'   => [
        ...
        $repeater->add_control('img_layers_rate',
            [
                'label'       => esc_html__('Rate', 'rehomes-core'),
                'type'        => Controls_Manager::NUMBER,
                'default'     => -10,
                'min'         => -20,
                'max'         => 20,
                'step'        => 1,
                'description' => esc_html__('Choose the movement rate for the layer image, default: -10', 'rehomes-core'),
                'separator'   => 'after',
                'label_block' => true,
                'condition'   => [
                    'img_layers_mouse'      => 'yes',
        ...
        $repeater->add_control('img_layers_zindex',
            [
                'label'     => esc_html__('z-index', 'rehomes-core'),
                'type'      => Controls_Manager::NUMBER,
                'default'   => 1,
                'selectors' => [
        ...
        $repeater->add_control('img_layers_class',
            [
                'label'       => esc_html__('CSS Classes', 'rehomes-core'),
                'type'        => Controls_Manager::TEXT,
                'description' => esc_html__('Separate class with spaces', 'rehomes-core'),
            ]
        );
                ...
                foreach ($settings['img_layers_images_repeater'] as $index => $image) :
                    ...
                    <li <?php echo $this->get_render_attribute_string($list_item_key); ?> <?php if ('yes' == $image['img_layers_mouse']) : echo 'data-' . $image['img_layers_mouse_type'] . '="true"'; ?> data-rate="<?php echo esc_attr(!empty($image['img_layers_rate']) ? $image['img_layers_rate'] : -10); ?>"<?php endif; ?>>
                        ...
                        if ($image['layers_select'] === 'image') {
                            ...
                            if (empty($image_src_size)) : $image_src_size = $image_src['url'];endif;
