...
if (!function_exists('osf_cart_link')) {
    ...
    function osf_cart_link()
    {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            $items = '';
            $items .= '<a data-toggle="toggle" class="cart-contents header-button" href="' . esc_url(wc_get_cart_url()) . '" title="' . __("View your shopping cart", "rehomes-core") . '">';
...
if (!function_exists('osf_cart_count_text')) {
    ...
    {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            return '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "rehomes-core")) . '</span>';
        }
...
if (!function_exists('osf_handheld_footer_bar_search')) {
    ...
    {
        echo '<a href="">' . esc_attr__('Search', 'rehomes-core') . '</a>';
        osf_product_search();
    }
}
...
if (!function_exists('osf_handheld_footer_bar_cart_link')) {
    ...
    function osf_handheld_footer_bar_cart_link()
    {
        ...
        <a class="footer-cart-contents" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'rehomes-core'); ?>">
...
if (!function_exists('osf_handheld_footer_bar_account_link')) {
    ...
    {
        echo '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'rehomes-core') . '</a>';
    }
}
    ...
    function osf_woocommerce_product_best_selling()
    {
        ...
        <div class="best-selling">
            <div class="best-selling-inner">
                <h4 class="best-selling-title"><?php echo esc_html__('Trending', 'rehomes-core'); ?></h4>
                <ul class="product_list_widget product-best-selling">
                    ...
                    if ($loop->have_posts()) {
                        ...
                        echo __('No products found', 'rehomes-core');
                    }
                    wp_reset_postdata();
                    ?>
                </ul>
...
if (!function_exists('osf_woocommerce_order_review_heading')) {
    ...
    {
        echo ' <h3 class="order_review_heading">' . esc_attr__('Your order', 'rehomes-core') . '</h3>';
    }
}
...
if (!function_exists('osf_woocommerce_product_loop_action_start')) {
    ...
    {
        echo '<div class="group-action"><div class="shop-action">';
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_stock')) {
    function osf_woocommerce_get_product_label_stock()
    {
        ...
        if ($product->get_stock_status() == 'outofstock') {
            echo '<span class="stock-label">' . esc_html__('Out Of Stock', 'rehomes-core') . '</span>';
        } else {
            echo '<span class="stock-label">' . esc_html__('In Stock', 'rehomes-core') . '</span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_feature')) {
    function osf_woocommerce_get_product_label_feature()
    {
        ...
        if ($product->is_featured()) {
            echo '<span class="trend">' . esc_html__('Trend', 'rehomes-core') . '</span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_set_register_text')) {
    ...
    {
        echo '<div class="user-text">' . __("Creating an account is quick and easy, and will allow you to move through our checkout quicker.", "rehomes-core") . '</div>';
    }
}
...
if (!function_exists('osf_woocommerce_time_sale')) {
    function osf_woocommerce_time_sale()
    {
        ...
        if ($time_sale) {
            ...
            echo '<div class="time">
                    <div class="deal-text d-none">' . esc_html__('Time Left', 'rehomes-core') . '</div>
                    <div class="opal-countdown clearfix typo-quaternary"
                        data-countdown="countdown"
                        data-days="' . esc_html__("days", "rehomes-core") . '" 
                        data-hours="' . esc_html__("hours", "rehomes-core") . '"
                        data-minutes="' . esc_html__("mins", "rehomes-core") . '"
                        data-seconds="' . esc_html__("secs", "rehomes-core") . '"
                        data-Message="' . esc_html__('Expired', 'rehomes-core') . '"
...
if (!function_exists('osf_active_filters')) {
    function osf_active_filters()
    {
        ...
        echo '<span class="osf_active_filters_label">' . esc_html__('Active Filters: ', 'rehomes-core') . '</span>
    <a class="clear-all" href="' . esc_url($link_remove_all) . '">' . __('Clear Filters', 'rehomes-core') . '</a>
</div>';
    }
}
...
if (!function_exists('osf_single_product_social')) {
    function osf_single_product_social()
    {
        if (get_theme_mod('osf_socials')) {
            $template = REHOME_CORE_PLUGIN_DIR . 'templates/socials.php';
            $socials_label = true;
...
if (!function_exists('osf_single_product_quantity_label')) {
    ...
    {
        echo '<label class="quantity_label">' . __('Quantity', 'rehomes-core') . ' </label>';
    }
}
...
if (!function_exists('osf_woocommerce_deal_progress')) {
    function osf_woocommerce_deal_progress()
    {
        ...
        <div class="deal-sold">
            <span class="deal-text d-block"><span><?php esc_html_e('HURRY! ONLY', 'rehomes-core') ?></span> <span
                        class="c-primary"><?php echo esc_attr(trim($limit - $sold)) ?></span> <span><?php esc_html_e('LEFT IN STOCK.', 'rehomes-core') ?></span></span>
            <div>
...
function otf_woocommerce_recently_viewed_product()
{
    if (get_theme_mod('otf_woocommerce_extra_enable_product_recently_viewed', false)) {
        ...
        if (!empty($_COOKIE['otf_woocommerce_recently_viewed'])) {
            ...
            echo '<h2 class="otf-woocommerce-recently-viewed">' . esc_html__('Your Recently Viewed', 'rehomes-core') . '</h2>';
...
function osf_filter_woocommerce_loop_add_to_cart_link($quantity, $product)
{
    return '<div class="opal-add-to-cart-button">' . $quantity . '</div>';
}

// add the filter
add_filter('woocommerce_loop_add_to_cart_link', 'osf_filter_woocommerce_loop_add_to_cart_link', 10, 2);

function osf_woocommerce_pagination_args($args)
{
    $args['prev_text'] = '<span class="opal-icon-chevron-left"></span>';
    $args['next_text'] ='<span class="opal-icon-chevron-right"></span>';
    $args['type'] = 'plain';
    return $args;
}
