...
class Opal_Woocommerce_Extra {
    ...
    public function __construct() {
        ...
        include_once(REHOME_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-layered-nav.php');
    ...
    public function enqueue_scripts($hook) {
        if ($hook === 'post.php' && get_post_type() === 'product') {
            wp_enqueue_script('otf-woocommerce-admin', REHOME_CORE_PLUGIN_URL . 'assets/js/woocommerce/admin.js', array('jquery'), REHOME_CORE_VERSION, true);
        }
    }
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'rehomes-core') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'rehomes-core')); ?>
            <input type="number" min="<?php echo $min; ?>" class="short" name="_deal_quantity" id="_deal_quantity"
                   value="<?php echo esc_attr($quantity) ?>">

            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts" style="clear:both;display:block;"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'bicomart-core', 'filaminimal-core', 'striz-core', 'striz-core', 'striz-core', 'striz-core', 'rehomes-core', 'rehomes-core', 'rehomes-core'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
