...
//                add_action('woocommerce_after_shop_loop', 'woocommerce_result_count', 5);
		...
		public function mobile_handheld_footer_bar() {
			$links = array(
				'my-account' => '<a class="my-accrount-footer" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'rehomes-core' ) . '</a>',
				'search'     => '<a class="search-footer" href="">' . esc_attr__( 'Search', 'rehomes-core' ) . '</a><div class="site-search">' . get_search_form( false ) . '</div>',
				'cart'       => ' <a class="footer-cart-contents" href="' . esc_url( wc_get_cart_url() ) . '" 
                title="' . esc_attr__( 'View your shopping cart', 'rehomes-core' ) . '">  
 <span class="count">' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</span>
		...
		public function label_tooltip() {
			...
			echo '<div id="osf-woocommerce-cart">' . esc_html__( 'Add to cart', 'rehomes-core' ) . '</div>';
			echo '</div>';
		}
		...
		public function added_to_cart_template() {
			$text = esc_html__( 'has been added to your cart', 'rehomes-core' );
			echo <<<HTML
...
HTML;
		...
		public function widgets_init() {
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop', 'rehomes-core' ),
				'id'            => 'sidebar-woocommerce-shop',
				'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'rehomes-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Detail', 'rehomes-core' ),
...
//            register_sidebar(array(
//                'name' => __('WooCommerce Shop Filters', 'rehomes-core'),
...
//            ));
		...
		public function render_woocommerce_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop' ) && osf_is_product_archive() ) {
				echo '<div id="opal-canvas-filter" class="opal-canvas-filter position-' . $position . '"><span class="filter-close">' . esc_html__( 'CLOSE', 'rehomes-core' ) . '</span><div class="opal-canvas-filter-wrap">';
		...
		public function render_button_shop_canvas() {
			if ( is_active_sidebar( 'sidebar-woocommerce-shop' ) && osf_is_product_archive() ) {
				echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__( 'Filter', 'rehomes-core' ) . '</button>';
			}
		}
			...
			if ( ! empty( $query ) ) {
				...
				if ( is_object( $product_object ) ) {
					...
					if ( ! empty( $product_sku ) ) {
						$product_sku_display = ' - ' . __( 'Sku', 'rehomes-core' ) . ': ' . $product_sku;
					}

					$product_title_display = '';
					if ( ! empty( $product_title ) ) {
						$product_title_display = ' - ' . __( 'Title', 'rehomes-core' ) . ': ' . $product_title;
					}

					$product_id_display = __( 'Id', 'rehomes-core' ) . ': ' . $product_id;
		...
		public function productCategoryAutocompleteRender( $query ) {
			...
			$term_slug_display = '';
			if ( ! empty( $term_slug ) ) {
				$term_slug_display = ' - ' . __( 'Sku', 'rehomes-core' ) . ': ' . $term_slug;
			}

			$term_title_display = '';
			if ( ! empty( $term_title ) ) {
				$term_title_display = ' - ' . __( 'Title', 'rehomes-core' ) . ': ' . $term_title;
			}

			$term_id_display = __( 'Id', 'rehomes-core' ) . ': ' . $term_id;
		...
		public function taxonomy_metaboxes() {
			...
			$cmb_term = new_cmb2_box( array(
				'id'           => 'product_cat',
				'title'        => __( 'Product Metabox', 'rehomes-core' ), // Doesn't output for term boxes
		...
		public function product_video_custom_field() {
			...
			$cmb    = new_cmb2_box( array(
				...
				'title'        => esc_html__( 'Product Video Config', 'rehomes-core' ),
			...
			$cmb->add_field( array(
				'name' => __( 'Product video', 'rehomes-core' ),
				'desc' => __( 'Supports video from youtube and vimeo.', 'rehomes-core' ),
				'id'   => $prefix . 'video',
				'type' => 'oembed',
			) );

			$cmb->add_field( array(
				'name'         => __( 'Video Thumbnail', 'rehomes-core' ),
			...
			if ( isset( $atts['product_layout'] ) && $atts['product_layout'] != 'grid' ) {
				...
				if ( $atts['product_layout'] === 'list' || $atts['product_layout'] === 'list-carousel' ) {
					...
					if ( ! empty( $atts['show_rating'] ) ) {
						add_action( 'osf_product_list_before_price', 'osf_woocommerce_list_show_rating', 15 );
					}
					...
					if ( ! empty( $atts['show_time_sale'] ) ) {
						add_action( 'osf_product_list_after_price', 'osf_woocommerce_time_sale', 20 );
					}
		...
		public function style_loop_end( $atts = array() ) {
			if ( isset( $atts['product_layout'] ) && $atts['product_layout'] != 'grid' ) {
				if ( $atts['product_layout'] === 'list' ) {
					...
					if ( ! empty( $atts['show_rating'] ) ) {
						remove_action( 'osf_product_list_before_price', 'osf_woocommerce_list_show_rating', 15 );
					}
					...
					if ( ! empty( $atts['show_time_sale'] ) ) {
						remove_action( 'osf_product_list_after_price', 'osf_woocommerce_time_sale', 20 );
					}
				}
			}
		}
		...
		public function woocommerce_scripts() {
			...
			wp_enqueue_script( 'osf-woocommerce-main', trailingslashit( REHOME_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/main.js', array(
				'jquery',
				'wp-util'
			), false, true );
			wp_localize_script( 'osf-woocommerce-main', 'qos', array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
			) );

			if ( is_product() ) {
				wp_enqueue_script( 'osf-woocommerce-single', trailingslashit( REHOME_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/single.js', array(), false, true );
				if ( get_theme_mod( 'osf_woocommerce_single_product_tab_style', 'tab' ) == 'accordion' ) {
					wp_enqueue_script( 'osf-woocommerce-single-accordion', trailingslashit( REHOME_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true );
				}
			}
		...
		public function custom_comment_form( $comment_form ) {
			...
			$comment_form['fields']        = array(
				'author' => '<p class="comment-form-author"><input id="author" name="author" type="text" placeholder="' . esc_attr__( "Name", "rehomes-core" ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" required /></p>',
				'email'  => '<p class="comment-form-email"><input id="email" name="email" type="email" placeholder="' . esc_attr__( "Email", "rehomes-core" ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" required /></p>',
			);
			$comment_form['comment_field'] = '';

			if ( wc_review_ratings_enabled() ) {
				$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'rehomes-core' ) . '</label><select name="rating" id="rating" required>
						<option value="">' . esc_html__( 'Rate&hellip;', 'rehomes-core' ) . '</option>
						<option value="5">' . esc_html__( 'Perfect', 'rehomes-core' ) . '</option>
						<option value="4">' . esc_html__( 'Good', 'rehomes-core' ) . '</option>
						<option value="3">' . esc_html__( 'Average', 'rehomes-core' ) . '</option>
						<option value="2">' . esc_html__( 'Not that bad', 'rehomes-core' ) . '</option>
						<option value="1">' . esc_html__( 'Very poor', 'rehomes-core' ) . '</option>
					</select></div>';
			}


			$comment_form['comment_field'] .= '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="45" rows="2" required placeholder="' . esc_attr__( "Your review", "rehomes-core" ) . '"></textarea></p>';

			$comment_form['label_submit'] = __( 'Submit Your Review', 'rehomes-core' );
