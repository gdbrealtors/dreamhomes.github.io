...
class OSF_Theme_Setup {
    ...
    public function __construct() {
        ...
        //
        //add_action('osf_single_entry_footer', array($this, 'render_related_posts'));
    }

    public function scripts() {
        wp_enqueue_style('opal-custom-css', trailingslashit(REHOME_CORE_PLUGIN_URL) . 'assets/css/custom.css', [], REHOME_CORE_VERSION);
    }

    public function widgets_init() {
        register_sidebar(array(
            'name'          => esc_html__('Single Posts Sidebar', 'rehomes-core'),
            'id'            => 'sidebar-single-post',
            'description'   => esc_html__('Add widgets here to appear in your sidebar on single posts.', 'rehomes-core'),
        ...
        register_sidebar(array(
            'name'          => esc_html__('Blog Sidebar', 'rehomes-core'),
            'id'            => 'sidebar-blog',
            'description'   => esc_html__('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'rehomes-core'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title">',
            'after_title'   => '</h2>',
        ...
        register_sidebar(array(
            'name'          => esc_html__('Page Sidebar', 'rehomes-core'),
            'id'            => 'sidebar-page',
            'description'   => esc_html__('Add widgets here to appear in your pages.', 'rehomes-core'),
            'before_widget' => '<section id="%1$s" class="widget %2$s">',
            'after_widget'  => '</section>',
            'before_title'  => '<h2 class="widget-title">',
            'after_title'   => '</h2>',
    ...
    public function modal_login_register() {
        include_once trailingslashit(REHOME_CORE_PLUGIN_DIR) . 'templates/modal/login-register-form.php';
    }
    ...
    private function set_main_nav_content($items) {
        ...
        $items .= '<a data-search-toggle="toggle" data-target=".' . $id . '" class="fa fa-search" title="' . __("search", "rehomes-core") . '"></a>';
...
HTML;
    ...
    public function render_related_posts() {
        if (is_active_sidebar('sidebar-single-post') || (!class_exists('RehomesCore') && is_active_sidebar('sidebar-blog'))) {
            rehomes_fnc_related_post(2);
        } else {
            rehomes_fnc_related_post(2);
        }
    }
